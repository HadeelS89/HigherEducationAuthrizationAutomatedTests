...
public class ProgramWithChangeERTest extends Base {
    LoginPage loginPage;
    HEProgram myProgram;
    DDActions ddActions;
    PCActions pcActions;
    ...
    @Test(description = "DD request to change ER ",
            retryAnalyzer = RetryAnalyzer.class,priority = 6)
    public void changeERByDD() throws Exception {
        //Login as Program Manager
        loginPage = new LoginPage(driver);
        loginPage.signInAsADEKEmployee(ReadWriteHelper.readCredentialsXMLFile("DivisionDirector",
                "username"),
    ...
    @Test(description = "After request for change Select ER list by PC ",
            retryAnalyzer = RetryAnalyzer.class,priority = 7)
    public void selectERFromListAgain() throws Exception {
        //Login as Program Manager
        loginPage = new LoginPage(driver);
        loginPage.signInAsADEKEmployee(ReadWriteHelper.readCredentialsXMLFile("ProvisionCoord1",
                "username"),
        ...
        pcActions = new PCActions(driver);
        pcActions.findProgram(ReadWriteHelper.getHEApplication());
        pcActions.selectERList("RequestToChangeER", "NewERSelection");
    ...
    public void acceptInvitationByER() throws Exception {
        ...
        for (int j = 2; j <= numberOfER+1; j++) {
            ...
            ActionsHelper.waitToBeClickable(erActions.getLogoutDDL(),60);
    ...
    public void submitJointReport() throws Exception {
        ...
        loginPage.signIn(ReadWriteHelper.readFromExcel
                        ("ERHireEducation", "ERList", "ERLoginEmails", 2),
                ReadWriteHelper.readFromExcel
                        ("ERHireEducation", "ERList", "Password", 2), "HigherAdminURL");
